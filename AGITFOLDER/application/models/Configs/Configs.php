...
class Configs extends CI_Model {
  ...
  public function __construct() {
    ...
    */
    $this->banner = PHP_EOL."
{$this->yellow}                         \ \ {$this->red}|{$this->reset}%s
{$this->yellow}    .-'''''-.    __       |/ {$this->red}|{$this->yellow}-----------------------
{$this->yellow}   /         \.'`  `',.--//  {$this->red}|{$this->yellow}* BOT FOR FACEBOOK [{$this->red}CLI{$this->yellow}]
{$this->yellow} -(  {$this->red}FACEBOOK{$this->yellow} | {$this->red}BOT{$this->yellow}  |  {$this->red}@@{$this->yellow}\  {$this->red}|{$this->yellow}* AUTHOR {$this->red}:{$this->yellow} MrGhost
{$this->yellow}   \         /'.____.'\___|  {$this->red}|{$this->yellow}* System {$this->red}:{$this->yellow} Messenger
{$this->yellow} —————————————————— [INFORMATION] ——————————————————>
{$this->yellow} [{$this->green}✓{$this->yellow}] Creator {$this->red}:{$this->yellow} MrGhostOfficial{$this->red}[{$this->yellow}NetHunter{$this->red}]
{$this->yellow} [{$this->green}✓{$this->yellow}] YouTube {$this->red}:{$this->yellow} HackerFake424
{$this->yellow} [{$this->green}✓{$this->yellow}] Support {$this->red}:{$this->yellow} Nethantara CyberCoder TeM
{$this->yellow} [{$this->green}✓{$this->yellow}] GitHub  {$this->red}:{$this->yellow} https://github.com/MrGhostOfficial
{$this->yellow} ———————————————————————————————————————————————————>
{$this->reset}    v1.1 dev{$this->red} /   /  /        {$this->red}|*{$this->green} %s
{$this->red}                 \  \ \n{$this->reset}".PHP_EOL;
    /*
  ...
  public function load_cookies() {
    if (file_exists('logs/cookies.txt') and filesize('logs/cookies.txt') > 0) {
      /*
      @Read Cookies
      */
      $file = fopen('logs/cookies.txt', 'r');
      $cookies = fgets($file);
      fclose($file);
      return $cookies;
    } else
  ...
  public function request_get($url, $cookies = false) {
    ...
    if (curl_errno($ch)) {
      $this->climate->br()->shout('  Error, please check your connection try again!');
      exit(0);
    }
    curl_close($ch);
    return $response;
  ...
  public function show_menu() {
    $menu = json_encode(
      array(
        ...
        ['no' => '07', 'name' => 'Random Posting Caption'],
        ...
        ['no' => 'SC', 'name' => 'See Cookies'],
        ['no' => 'RC', 'name' => 'Remove Cookies'],
        ['no' => 'EX', 'name' => 'Exit tool program'],
      )
    );
